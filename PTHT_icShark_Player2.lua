-- PTHT Player 2 (Right to Left) Configuration
...
-- Ensure player faces left (for Player 2)
local function ensureFacingLeft()
    if GetWorld() == nil then
        return false
    end
    
    if GetLocal() and not GetLocal().facing_left then
...
local function playerHook(info)
    if whUse then
        ...
        local script = [[
            $webHookUrl = "]].. whUrl ..[["
            $title = "<a:TB_warning:1101039889170046997> **PTHT Player 2 Information** <a:TB_warning:1101039889170046997>"
      ...
      username = "PTHT Player 2 by @Mupnup"
...
playerHook("**Player 2 Script is now running!**")
Sleep(1000)
...
add_label_with_icon|big|`#[`bPTHT Player 2 `#] `2Settings|left|15757|
add_spacer|small|
...
overlayText("`7[`1P`cl`5a`#y`1e`cr `52 `#by `#@Mupnup`7]")
SendPacket(2, "action|input\ntext|`7[`1P`cl`5a`#y`1e`cr `52 `#by `b@Mupnup`7]")
logText("Player 2 Script is now running!")
Sleep(1000)
SendPacket(2, "action|input\ntext|`7[`1P`cl`5a`#y`1e`cr `52 `#by `b@Mupnup`7]")
overlayText("`7[`1P`cl`5a`#y`1e`cr `52 `#by `#@Mupnup`7]")
...
local function worldNot()
    if not GetWorld() or GetWorld().name ~= (worldName:upper()) then
        ...
        Sleep(delayRecon)
        playerHook("Player 2 Reconnected, looks like you were recently disconnected")
    else
        Sleep(delayRecon)
        checkRemote()
    end
...
local function harvest()
    ...
    while countReady() > 0 do
        ...
        for y = 0, yAxis do
            -- Changed: Harvest from right to left (xAxis to 0)
            for x = xAxis, 0, -1 do
            if GetWorld() == nil then
                return false
            end
...
local function plant()
    ...
    -- Changed: Plant from right to left (xAxis to 0)
    for x = xAxis, 0, -1 do
        ...
        for y = yStart, yEnd, yStep do
            ...
            if tile and aboveTile and tile.fg == 0 and aboveTile.fg == platformID then
                FindPath(x, y, 50)
                Sleep(delayPlant)
                -- Plant at current position (0,0 offset)
                place(5640, 0, 0)
                Sleep(delayPlant)
            end
        end
...
local function plantMissedSpots()
    ...
    -- Changed: Plant missed spots from right to left (xAxis to 0)
    for x = xAxis, 0, -1 do
        for y = 0, yAxis do
            if changeRemote or GetWorld() == nil then return false end
...
if changeRemote or noStock then
    ...
    if harvestdone then
        ...
        playerHook("Player 2 Harvested ".. harvestCount .." / ".. maxPTHT .." times")
        SendPacket(2, "action|input\ntext| `1P`c2 `5H`#A`1R`cV`5E`#S`1T`cE`5D `c[+] `5".. harvestCount .." / ".. maxPTHT .." `#TIMES")
        harvestdone = false
        Sleep(1000)
    end
    ...
    if harvestMax then
        playerHook("Player 2 DONE HARVESTED ".. maxPTHT .." times")
        SendPacket(2, "action|input\ntext|`1P`c2 `5D`#O`1N`cE `5H`#A`1R`cV`5E`#S`1T`cE`5D `c=> `5".. maxPTHT .." `#TIMES")
        Sleep(1000)
        break
    end
